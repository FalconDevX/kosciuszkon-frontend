...
function applyWhiteEdgesOnly(root: THREE.Object3D) {
    root.traverse((child) => {
        ...
        const lines = new THREE.LineSegments(edgesGeom, new THREE.LineBasicMaterial({ color: 0xffffff }));
        mesh.add(lines);
...
function LoadingPlaceholder() {
    ...
    const edgeGeo = useMemo(() => new THREE.EdgesGeometry(new THREE.BoxGeometry(0.45, 0.45, 0.45), 18), []);
    useFrame((_, delta) => {
...
export function HomeLoggedInLaptop({ dictionary, locale }: Props) {
    ...
    return (<div className="w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-900/75 p-7 shadow-[0_24px_70px_-38px_rgba(37,99,235,0.5)] backdrop-blur-xl">
      ...
      <div className="relative mt-6 h-[min(42vh,320px)] w-full min-h-[220px] overflow-hidden rounded-xl bg-zinc-950/60">
        <Canvas gl={{ alpha: true, antialias: true }} className="h-full w-full touch-none" camera={{ position: [0, 0.12, 2.35], fov: 42 }}>
          <Suspense fallback={<LoadingPlaceholder />}>
            <RotatingLaptopModel />
          </Suspense>
        </Canvas>
      </div>
      <Link href={`/${locale}/dashboard`} className={cn(buttonVariants({ variant: "default", size: "lg" }), "mt-6 h-11 w-full cursor-pointer bg-blue-500 text-zinc-950 transition-colors duration-200 hover:bg-blue-950! hover:text-white!")}>
