...
const HomeLoggedInLaptop = dynamic(() => import("@/app/components/home/HomeLoggedInLaptop").then((m) => ({
    ...
    return (<main className="relative min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <section className="relative h-[calc(100vh-4rem)] overflow-hidden px-6 py-6 md:px-10">
        ...
        <div className="relative z-10 mt-4 flex w-full items-center justify-center md:mt-6">
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, ease: "easeOut" }} className="mb-2 w-full text-center font-mono text-2xl font-bold tracking-tight text-zinc-100 md:text-4xl">
            {isTypingFinished ? highlightedHeadline : typingHeadline}
            {!isTypingFinished ? (<span className="ml-1 inline-block h-[1em] w-[2px] animate-pulse bg-zinc-100 align-middle"/>) : null}
          </motion.h1>
        </div>
        <div className="relative z-10 mx-auto flex h-[calc(100vh-10rem)] w-full max-w-[1280px] flex-col items-start justify-center gap-4 lg:-translate-y-10 lg:flex-row lg:items-center xl:gap-8">
          <motion.div initial={{ opacity: 0, y: 60 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: "easeOut" }} className="hidden lg:flex lg:flex-col lg:items-start lg:justify-center lg:gap-6">
            <Image src="/safe_click_dark_small.png" alt="SafeClick shield logo" width={240} height={240} className="h-auto w-full max-w-[210px] self-center will-change-transform" style={{ animation: "float-soft-a 6.6s ease-in-out infinite" }} priority/>
            <Image src="/safe_click_dark_title.png" alt="SafeClick title logo" width={640} height={142} className="h-auto w-full max-w-[440px] will-change-transform" style={{ animation: "float-soft-b 7.2s ease-in-out infinite" }} priority/>
            <div className="max-w-[560px] text-left">
              <ul className="mt-1 space-y-2 text-base text-zinc-300">
                {featureHighlights.map((feature, index) => (<motion.li key={feature} className="flex items-start" initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{
                duration: 0.45,
          ...
          <motion.div initial={{ opacity: 0, y: 60 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.14, ease: "easeOut" }}>
            {storedUserState === "loading" ? (<div className="w-full max-w-md min-h-[min(520px,70vh)] rounded-2xl border border-zinc-800/40 bg-zinc-900/40" aria-hidden/>) : storedUserState === "in" ? (<HomeLoggedInLaptop dictionary={dictionary} locale={locale}/>) : (<AuthPanel dictionary={dictionary} locale={locale}/>)}
          </motion.div>
        </div>
      </section>
