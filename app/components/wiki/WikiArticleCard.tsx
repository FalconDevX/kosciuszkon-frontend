...
export function WikiArticleCard({ article, selected, onSelect, dangerLevelLabel, }: WikiArticleCardProps) {
    return (<motion.button type="button" whileHover={{ y: -1 }} onClick={onSelect} className={`w-full rounded-xl border p-4 text-left transition ${selected
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] ${dangerBadgeClass(article.dangerLevel)}`}>
