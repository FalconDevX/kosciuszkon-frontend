...
export function CyberWikiDashboard({ locale, dictionary }: Props) {
    ...
    useEffect(() => {
        if (!articleFromUrl)
            return;
        if (!filteredArticles.some((item) => item.id === articleFromUrl))
            return;
    ...
    return (<main className="relative min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <section className="relative overflow-hidden px-4 pb-8 pt-6 md:px-6">
        ...
        <div className="relative z-10 mx-auto w-full max-w-[1450px] space-y-5">
          <motion.header initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-zinc-800/80 bg-zinc-900/55 p-6 backdrop-blur">
            ...
            <div className="mt-4">
              <WikiSearch value={searchQuery} onChange={setSearchQuery} placeholder={wiki.searchPlaceholder}/>
            </div>
          </motion.header>
