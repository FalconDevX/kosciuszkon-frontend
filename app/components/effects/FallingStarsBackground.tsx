...
export function FallingStarsBackground({ className, density = "default" }: Props) {
    ...
    return (<div className={clsx("sparkle-orb-layer pointer-events-none absolute inset-0 overflow-hidden", className)} aria-hidden>
      {orbs.map((o) => (<span key={o.id} className={clsx("sparkle-orb", o.variant === "a" ? "sparkle-orb--a" : "sparkle-orb--b")} style={{
                left: `${o.leftPct}%`,
                top: `${o.topPct}%`,
                width: `${o.sizePx}px`,
                height: `${o.sizePx}px`,
