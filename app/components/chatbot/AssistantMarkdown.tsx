...
const RISK_LEVEL_BADGE: Record<string, string> = {
    ...
    medium: "rounded px-1.5 py-0.5 font-semibold uppercase tracking-wide bg-amber-500/15 text-amber-200 ring-1 ring-amber-500/40",
    ...
    critical: "rounded px-1.5 py-0.5 font-semibold uppercase tracking-wide bg-red-500/15 text-red-300 ring-1 ring-red-500/45",
    unknown: "rounded px-1.5 py-0.5 font-semibold uppercase tracking-wide bg-zinc-500/20 text-zinc-300 ring-1 ring-zinc-500/40",
};
const RISK_SEGMENT = /(\*{0,2}?(?:risk|ryzyko)(?:\s+level|\s+poziom)?\*{0,2}?\s*:\s*\*{0,2}?)(low|medium|high|critical|unknown)\b\*{0,2}?/gi;
type Segment = {
    kind: "md";
    text: string;
} | {
...
const markdownComponents: Components = {
    p: ({ children }) => (<p className="mb-2 text-pretty leading-relaxed text-zinc-100 last:mb-0">{children}</p>),
    ...
    table: ({ children }) => (<div className="chatbot-scrollbar my-2 overflow-x-auto rounded-lg border border-zinc-700">
    ...
    pre: ({ children }) => (<pre className="chatbot-scrollbar my-2 overflow-x-auto rounded-lg border border-zinc-700 bg-zinc-950 p-3 text-xs text-zinc-200">
      {children}
    </pre>),
    code: ({ className, children, ...props }) => {
        const inline = !className;
        if (inline) {
            return (<code className="rounded bg-zinc-950 px-1 py-0.5 font-mono text-[0.9em] text-emerald-200/95" {...props}>
          {children}
        </code>);
        }
        return (<code className={`font-mono text-[0.9em] ${className ?? ""}`} {...props}>
        {children}
      </code>);
    },
};
export function AssistantMarkdown({ content }: {
    ...
    return (<div className="assistant-markdown space-y-1 text-sm [&>:first-child>*:first-child]:mt-0 [&>:last-child>*:last-child]:mb-0">
      {segments.map((seg, index) => {
            if (seg.kind === "risk") {
                const badgeClass = RISK_LEVEL_BADGE[seg.level] ?? RISK_LEVEL_BADGE.unknown;
                return (<div key={`risk-${index}`} className="py-0.5">
            ...
            if (!seg.text)
                return null;
            return (<div key={`md-${index}`} className="min-w-0">
