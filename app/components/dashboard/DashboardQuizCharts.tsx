    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-zinc-800/80 bg-zinc-950/40 p-4 backdrop-blur">
          ...
          {barMetrics ? (
            <svg
              viewBox={`0 0 ${barMetrics.w} ${barMetrics.h}`}
              className="max-h-[220px] h-auto w-full"
              role="img"
              aria-label={dictionary.pointsChartTitle}
            >
              <text
                x={barMetrics.padding.left - 6}
                y={barMetrics.padding.top + 4}
              ...
              <line
                x1={barMetrics.padding.left}
                y1={barMetrics.baselineY}
                x2={barMetrics.w - barMetrics.padding.right}
                y2={barMetrics.baselineY}
                stroke="rgb(63 63 70)"
                strokeWidth={1}
              />
              {barMetrics.bars.map((b) => (
                <g key={b.id}>
        ...
        <div className="rounded-2xl border border-zinc-800/80 bg-zinc-950/40 p-4 backdrop-blur">
          ...
          {lineMetrics ? (
            <svg
              viewBox={`0 0 ${lineMetrics.w} ${lineMetrics.h}`}
              className="max-h-[220px] h-auto w-full"
              role="img"
              aria-label={dictionary.progressChartTitle}
            >
              <text
                x={lineMetrics.padding.left - 8}
                y={lineMetrics.padding.top + 4}
                fill="rgb(113 113 122)"
                fontSize={10}
                textAnchor="end"
              >
              ...
              <line
                x1={lineMetrics.padding.left}
                y1={lineMetrics.baselineY}
                x2={lineMetrics.w - lineMetrics.padding.right}
                y2={lineMetrics.baselineY}
                stroke="rgb(63 63 70)"
                strokeWidth={1}
              />
              <path
                d={lineMetrics.linePath}
                fill="none"
              ...
              />
              {lineMetrics.dots.map((c) => (
                <circle key={c.id} cx={c.x} cy={c.y} r={4} fill="rgb(52 211 153)" opacity={0.95} />
              ))}
              <text
                x={lineMetrics.w - lineMetrics.padding.right}
                y={lineMetrics.padding.top + 12}
