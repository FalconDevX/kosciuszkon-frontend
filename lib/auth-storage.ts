...
function dispatchAuthChange(userId: string | null): void {
    if (typeof window === "undefined")
        return;
    window.dispatchEvent(new CustomEvent<string | null>(SAFECLICK_AUTH_EVENT, { detail: userId }));
}
...
export function subscribeAuthChange(callback: (userId: string | null) => void): () => void {
    if (typeof window === "undefined")
        return () => undefined;
    ...
    const onStorage = (event: StorageEvent) => {
        if (event.key !== SAFECLICK_USER_ID_KEY)
            return;
